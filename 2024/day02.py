...
def is_list_valid(report_list: List[int]) -> bool:
    ...
    is_increasing = all(report_list[i] <= report_list[i+1] for i in range(len(report_list)-1))
    is_decreasing = all(report_list[i] >= report_list[i+1] for i in range(len(report_list)-1))
